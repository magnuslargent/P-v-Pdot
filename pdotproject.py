...
psr_name = param_dict['PSRJ']
psr_ra = input('RA: ') #'20:27:16.233'
print(psr_ra)
psr_dec = input('DEC: ') #'+45:57:57.08'
print(psr_dec)
...
# P-Pdot diagram
uniquePeriod = float(input('Period: '))
print(uniquePeriod)
uniquePdot = float(input('Pdot = '))
print(uniquePdot)
ax1 = plt.subplot(121)
ax1.scatter(pers, pdots,c= '#377eb8',s=10)
#ax1.scatter([psr_period.value], [psr_pdot], c='r', label=psr_name)
ax1.scatter(uniquePeriod,uniquePdot, c ='r',marker = '^',label = 'Your Pulsar')
...
ax1.set_ylabel('Period derivative (s s$^{-1}$)', fontsize=20)
ax1.legend()
...
for i in range(len(names)):
    ...
    b_list.append(b_rad)
ax2.grid(True)
ax2.scatter(l_list, b_list,s =10)
...
ax2.scatter(psr_l_rad, psr_b_rad, c='r',marker = '^', label='Your Pulsar')
ax2.legend()
